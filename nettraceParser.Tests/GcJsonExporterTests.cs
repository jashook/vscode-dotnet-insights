...
public class GcJsonExporterTests
{
    ...
    private static JsonObject WriteAndParse(List<GcEvent> gcEvents)
    {
        ...
        try
        {
            GcJsonExporter.WriteToFile(outputPath, gcEvents, new List<AllocationEvent>(), new List<ExceptionEvent>(), new EventOverview(0, new List<EventTypeCount>()), new List<SampleEvent>(), new List<ContentionEvent>(), new DotnetInsights.NetTrace.Threading.ThreadingSummary(), new StackTable(), MethodSymbolTable.Build(new List<EventRecord>(), 8, 0, 0), "test-process", ticksBinaryPath, captureDurationMSec: 0, out _);
